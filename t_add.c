 /*t_add.c | RLotto | gcc | v0.8.353.1712
	...
	do
    {
		...
		for(i = 0; i <= T_Max_Row - 1; i++) {
			...
			if(countInput == 6 && checksum != (i+1) * 6){
				...
				checksum = 0;
				t_initialize();										//re-initialize to ensure structure is clean
				break;
			}
		}
...
void write_Ticket(void){
	...
	// construct filename
	char t_full_path[45];
	strcpy(t_full_path, TicketFolder);
	...
	if(fp == NULL) {
		...
		printf("\nTicket folder missing. Try to create now...\n");
			...
			if(pFile == NULL) {
				printf("Error opening %s for writing. Program terminated.", TicketFolder);
				abort();
			} else {
